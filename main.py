...
TOOL_DECLARATIONS = [
    {
        ...
        "description": (
            ...
            "Prefer THIS over computer_control's accomplish() for simply launching an app — this is a "
            "deterministic native launch, not a UI-automation guess. "
    ...
    },
    {
        ...
        "description": (
            ...
            "Prefer THIS tool over computer_control's accomplish() for WhatsApp/Telegram/Discord/Signal/"
            "Instagram/Messenger — it's a dedicated, tuned automation path for these specific platforms; use "
            "accomplish() only for a messaging platform not in this list."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            ...
            "one specific existing file with a clear instruction — edit needs an explicit "
            "confirmed=true after the user actually says yes); Git (status, diff, recent commit log, "
            "listing branches, staging changes, or committing staged changes with an explicit "
            "message — commit needs an explicit confirmed=true after the user actually says yes; "
            "push/pull/fetch and any destructive/history-rewriting operation like force-push or a "
            "hard reset are permanently unsupported and will be refused, never attempted); production "
            "deployment (checking whether the live backend/frontend are up, deployment status, recent "
            "deployment history, deploying, restarting, or rolling back the production service — "
            "deploy/restart/rollback each need an explicit confirmed=true after the user actually "
            "says yes, and will honestly report [BLOCKED] if no deployment credentials are configured; "
            "a deploy is refused if it would ship a code edit from earlier in the same request that no "
            "test run has verified since). For anything this doesn't yet cover, use the specific "
            "existing tool for that instead (this "
            "expands over time — it will return [INCONCLUSIVE] with 'no known JARVIS capability' if "
            "the objective isn't covered yet, never a guess). Every objective must be concrete "
            "enough for JARVIS to act on without guessing — e.g. for a spreadsheet action say 'put "
            "it in cell A1', never just 'put it in Excel'; for a file action name the SPECIFIC "
            "file/folder ('delete notes.txt from my desktop', never 'delete the old files'); for a "
            "repository action name the SPECIFIC search term or file ('edit main.py to add "
            "logging', never 'fix the project'); for a commit, always include the exact message in "
            "quotes ('commit these changes with message \"Fix login bug\"', never just 'commit'); for "
            "a rollback, name the specific deploy to roll back to (check the deployment history first "
            "if the user doesn't already know it) — JARVIS will not guess 'the previous one'. "
            "JARVIS will not invent a cell/file/target/message/deploy you didn't specify. "
            ...
            "skip ahead or invent what a failed step should have produced. This is also how a full "
            "technical development task is expressed: for \"find why X is broken, fix it, run the "
            "tests, and commit the fix\", give 'objectives' as the ordered atomic steps (e.g. search "
            "for the relevant code, edit the specific file with a clear instruction, run the tests, "
            "stage the changes, commit with an explicit quoted message) — JARVIS will refuse to "
            "commit (returning [INCONCLUSIVE]) if a code edit in that same run was never followed by "
            "a passing test run, so always include a 'run the tests' step between the edit and the "
            "commit. "
    ...
    },
    {
        ...
        "description": (
            ...
            "automation browser. "
            "Prefer THIS tool over computer_control's accomplish()/click/type for anything happening INSIDE "
            "a browser page — smart_click/smart_type reason over the page's real DOM/accessibility tree, more "
            "reliable than a generic screen-level UI guess that has no visibility into webpage content. "
    ...
    },
    {
        ...
        "description": (
            ...
            "coordinates for ordinary UI tasks anymore. Three specific exceptions, each with a more reliable "
            "dedicated tool — use accomplish() only for a case NONE of these three cover: (1) simply launching an "
            "application — use open_app instead, a deterministic native launch, not a UI-automation guess; (2) "
            "sending a message via WhatsApp/Telegram/Discord/Signal/Instagram/Messenger — use send_message "
            "instead, a dedicated tuned automation path for those specific platforms; (3) anything happening "
            "INSIDE a browser page (clicking/typing/reading a webpage's own content) — use browser_control's "
            "smart_click/smart_type instead, which reason over the page's real DOM, not a screen guess. "
            "It always returns one clear tagged result — "
