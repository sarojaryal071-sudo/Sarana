...
# Every action name computer_settings() actually accepts, ACTION_MAP's
# own keys plus the ones dispatched via a dedicated branch instead (see
# that branch list's own comment above) — the single source of truth
# main.py's own tool declaration enum is built from, and what
# _detect_action() below matches against.
_ALL_ACTIONS = sorted(set(ACTION_MAP.keys()) | {
    "volume_set", "app_volume_set", "app_mute", "app_unmute", "list_audio_devices",
    "bluetooth_on", "bluetooth_off", "sleep", "clipboard_get", "clipboard_set",
    "system_shortcut", "list_system_shortcuts", "type_text", "press_key", "reload_n",
})


def _detect_action(description: str) -> dict:
    """Deterministic, local action-name resolution — real, confirmed bug
    fixed: this used to spend an entire extra Gemini round trip
    translating free text into one of this module's own action names,
    because main.py's own tool declaration only ever said "the action to
    perform" instead of enumerating them (it now declares a real `enum`
    of every name in _ALL_ACTIONS). With the full vocabulary declared
    there, Gemini sends `action` directly almost every time now;
    `description` is the rare fallback, and difflib's plain string
    similarity against the real action vocabulary absorbs a spelling/
    translation near-miss in microseconds, no network call. It can't do
    true intent classification from unrelated free text — nothing local
    can — so a description with no lexical resemblance to any action
    name falls through to the same honest normalize-and-guess
    computer_settings()'s own dispatch already reports "Unknown action"
    for, exactly like before."""
    norm = re.sub(r"[^a-z0-9]+", "_", description.lower()).strip("_")
    match = difflib.get_close_matches(norm, _ALL_ACTIONS, n=1, cutoff=0.4)
    action = match[0] if match else norm
    num = re.search(r"\d+", description)
    return {"action": action, "value": int(num.group()) if num else None}
